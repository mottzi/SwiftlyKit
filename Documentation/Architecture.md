...
establishes the package and target. Preparation turns the accepted assessment
into a `LocalBuildEnvironment`, an immutable capability that retains the exact
package, target, Swiftly executable, toolchain, and SDK required by later calls.
...
- `Process` is the only adapter to `swift-subprocess`. Production uses
  `LiveSubprocessRunner`; tests use small recording adapters through the same seam.
